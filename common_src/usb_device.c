...
//Deprecated in v2.2 - will be removed in a future revision
...
void USBDeviceInit(void)
{
    ...
    for(uint8_t i = 0; i < (sizeof(BDT)/sizeof(BDT_ENTRY)); i++)
    {
        BDT[i].Val = 0x00;
    }
    ...
    for(uint8_t i = 0; i < (uint8_t)(USB_MAX_EP_NUMBER+1u); i++)
    {
        pBDTEntryIn[i] = 0u;
        pBDTEntryOut[i] = 0u;
        ep_data_in[i].Val = 0u;
...
static void USBStdFeatureReqHandler(void)
{
       ...
       (USBDeviceState == CONFIGURED_STATE))
    {
        ...
        else
        {
            ...
            #else
                pUEP = (uint8_t*)(&U1EP0+SetupPkt.EPNum);
            #endif
